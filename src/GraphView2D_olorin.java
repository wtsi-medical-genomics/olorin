 ...
 * @version 0.1
 */
public class GraphView2D_olorin extends GraphView {

    private AffineTransform transform;

    public Vector<NodeView> visibleNodes = new Vector<NodeView>();
	...
	GraphViewHandlerOlorin handler = new GraphViewHandlerOlorin();
//        GraphViewHandler handler = new GraphViewHandler();
     ...
     * @param graph
     */
    public GraphView2D_olorin(LayoutedGraph graph) {
	this();
	setGraph(graph);
    }
    ...
    public Point2D mouseToNode(int x, int y) {
	if (transform != null) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (isSelectionEnabled() && graph.getAllNodes() != null) {
			...
			if (nodeview.getBounds().contains(point)) {
			    if (nodeview.isSelected()) {
				System.out.println("GraphViewHandler");
    ...
    // olorin version
    class GraphViewHandlerOlorin extends GraphViewHandler implements ComponentListener, MouseListener, MouseMotionListener {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (isSelectionEnabled() && graph.getAllNodes() != null) {
		    ...
		    for (NodeView nodeview : visibleNodes) {
			if (nodeview.getBounds().contains(point)) {
			    fireNodeEvent(new NodeEvent(g, nodeview.getNode(), NodeEvent.SELECTED));
                            setCursor(Cursor.HAND_CURSOR);
			    return;
			}
                    }
		    fireNodeEvent(new NodeEvent(g, null, NodeEvent.ALL_DESELECTED));
		}
	    }
	}
    }
